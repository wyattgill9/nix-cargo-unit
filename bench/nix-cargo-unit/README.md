# `bench/nix-cargo-unit/` — rust-analyzer, one Cargo unit at a time
...
| `../rust-analyzer/` | a **git submodule** of `rust-lang/rust-analyzer`, untouched — shared with the buck2 bench |
...
`nix build ./bench/nix-cargo-unit` resolves to a plain `git+file://` fetch of this repository,
